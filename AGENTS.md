...
- **Backend services (Supabase, Firebase)** are hosted externally. API keys are embedded in the codebase for the Supabase anon key and Firebase config. No local database setup is needed.
- **`firebase/functions/`** contains a stub Cloud Functions project with no exported functions. It can be ignored for typical development.
