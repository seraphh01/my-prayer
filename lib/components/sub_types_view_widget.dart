...
class _SubTypesViewWidgetState extends State<SubTypesViewWidget> {
  ...
  Widget _buildPrayerRow(BuildContext context, PrayerStruct prayer) {
    return Padding(
      padding: const EdgeInsetsDirectional.fromSTEB(0.0, 0.0, 1.0, 2.0),
      child: InkWell(
        ...
        onTap: () async {
          ...
          safeSetState(() {});
          await widget.onSelectPrayer?.call(prayer.id);
        },
        child: _buildNavListTile(
          context: context,
          title: _prayerTitle(prayer),
          subtitle: _prayerSubtitle(prayer),
        ...
        if (prayers.isNotEmpty)
          Padding(
            ...
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                for (final prayer in prayers) _buildPrayerRow(context, prayer),
              ],
            ),
          ),
        if (subTypesItem.subtypes.isNotEmpty)
          Padding(
            ...
            child: SubTypesViewWidget(
              key: Key('Keyf0l_${subTypesItem.id}'),
              prayerTypes: subTypesItem.subtypes,
              expandAll: widget.expandAll,
              onSelectPrayer: widget.onSelectPrayer,
            ),
          ),
      ],
    );
  ...
  Widget _buildTypeRow(
    ...
    final isExpanded = widget.expandAll ||
    ...
    return Column(
      ...
      children: [
        ...
        if (isExpanded)
          _buildExpandedContent(context, subTypesItem, subTypesIndex),
      ],
    );
  }
    ...
    return Column(
      ...
      children: [
        ...
        for (var i = 0; i < subTypes.length; i++)
          _buildTypeRow(context, subTypes[i], i),
      ],
    );
  }
}
